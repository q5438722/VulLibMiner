...
def createDataCSV(dataset):
    ...
    with open(f'./data/{dataset}/valid{fext}') as f:
        for i in tqdm.tqdm(f):
            texts.append(i.replace('\n', ''))
            dataType.append('valid')
    ...
    with open(f'./data/{dataset}/valid_labels.txt') as f:
        for i in tqdm.tqdm(f):
            for l in i.replace('\n', '').split():
                label_map[l] = 0
            labels.append(i.replace('\n', ''))
    ...
    # for lib in maven_corpus:
    #     # print('maven:'+lib['name'])
    #     label_map['maven:'+lib['name']] = 0
    # print(len(label_map))
