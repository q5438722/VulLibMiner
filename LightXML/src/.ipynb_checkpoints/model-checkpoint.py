...
model_base_path = '/home/chentianyu/raw_models/'


def get_bert(bert_name):
    if 'roberta' in bert_name:
        print('load roberta-base')
        model_config = RobertaConfig.from_pretrained(model_base_path + 'roberta-base')
        model_config.output_hidden_states = True
        bert = RobertaModel.from_pretrained(model_base_path + 'roberta-base', config=model_config)
    elif 'xlnet' in bert_name:
        print('load xlnet-base-cased')
        model_config = XLNetConfig.from_pretrained(model_base_path + 'xlnet-base-cased')
        model_config.output_hidden_states = True
        bert = XLNetModel.from_pretrained(model_base_path + 'xlnet-base-cased', config=model_config)
    elif 'bigbird' in bert_name:
        print('load bigbird transformer')
        model_config = BigBirdConfig.from_pretrained("google/bigbird-roberta-base")
        model_config.output_hidden_states = True
        bert = BigBirdModel.from_pretrained('google/bigbird-roberta-base')
    elif 'longformer' in bert_name:
        print('load longformer transformer')
        model_config = RobertaConfig.from_pretrained('roberta-base')
        model_config.output_hidden_states = True
        bert = LongformerModel.from_pretrained('allenai/longformer-base-4096')
    elif 'docbert' in bert_name:
        print('load docbert from the checkpoint')
        model_config = BertConfig.from_pretrained('bert-base-uncased')
        model_config.output_hidden_states = True
        bert = BertModel.from_pretrained('docbert/')
    else:
        print('load bert-base-uncased')
        model_config = BertConfig.from_pretrained(model_base_path + 'bert-base-uncased')
        model_config.output_hidden_states = True
        bert = BertModel.from_pretrained(model_base_path + 'bert-base-uncased', config=model_config)
    return bert
...
class LightXML(nn.Module):
    def __init__(self, n_labels, group_y=None, bert='bert-base', feature_layers=1, dropout=0.5, update_count=1,
        ...
        if self.group_y is not None:
            ...
            print("SIZE OF LINEAR LAYER:")
            print(self.feature_layers*self.bert.config.hidden_size)
    ...
    def forward(self, input_ids, attention_mask, token_type_ids,
        ...
        if self.bert_name=="longformer" or self.bert_name=="bigbird":
            sentence_embeddings = outs
            temp_list = []
            for sent in sentence_embeddings:
                temp_list.append(sent)
            out = torch.stack(temp_list)
        else:
            word_embeddings = outs[-1]
            # loop through the second last to the fourth last dimension
            for i in range(0, len(outs) - 1):
                dimension_i_embedding = outs[-i]
                for j in range(0, len(word_embeddings)):
                    # print(sentence_embeddings[j])
                    word_embeddings[j] = word_embeddings[j] + dimension_i_embedding[j]
            #
            sentence_embeddings = []
            for i in range(0, len(word_embeddings)):
                # sentence_embeddings.append(self.mean_pooling(word_embeddings[i], attention_mask[i]))
                sentence_embeddings.append(torch.mean(word_embeddings[i], dim=0))
            out = torch.stack(sentence_embeddings)
    ...
    def get_fast_tokenizer(self):
        if 'roberta' in self.bert_name:
            tokenizer = RobertaTokenizerFast.from_pretrained(model_base_path + 'roberta-base', do_lower_case=True)
        elif 'xlnet' in self.bert_name:
            tokenizer = XLNetTokenizer.from_pretrained(model_base_path + 'xlnet-base-cased') 
        else:
            tokenizer = BertWordPieceTokenizer(model_base_path + 'bert-base-uncased/vocab.txt',
                lowercase=True)
        return tokenizer
    ...
    def get_tokenizer(self):
        if 'roberta' in self.bert_name:
            ...
            tokenizer = RobertaTokenizer.from_pretrained(model_base_path + 'roberta-base', do_lower_case=True)
        elif 'xlnet' in self.bert_name:
            print('load xlnet-base-cased tokenizer')
            tokenizer = XLNetTokenizer.from_pretrained(model_base_path + 'xlnet-base-cased')
        elif 'docbert' in self.bert_name:
            print("load docbert tokenizer")
            tokenizer = BertTokenizer.from_pretrained('docbert/')
        elif 'bigbird' in self.bert_name:
            print("load bigbird tokenizer")
            tokenizer = BigBirdTokenizer.from_pretrained('google/bigbird-roberta-base')
        elif 'longformer' in self.bert_name:
            print("load longformer tokenizer")
            tokenizer = LongformerTokenizer.from_pretrained('allenai/longformer-base-4096')
        else:
            print('load bert-base-uncased tokenizer')
            tokenizer = BertTokenizer.from_pretrained(model_base_path + 'bert-base-uncased', do_lower_case=True)
        return tokenizer
